...
"keep the harness local" option put `.claude/` in `.gitignore`. `.claude/CUSTOM.md`'s
`## Harness settings` records which as a `- Harness: committed | local` line (fall back to
...
| Behavioral preference | `.claude/CUSTOM.md` (never the synced `.claude/CLAUDE.md`)                                                         | `~/.claude/CLAUDE.md`      |
...
1. Parse the user's description and classify it:
   ...
   **Behavioral instruction** (a CLAUDE.md):
   ...
   - In a container this lands in `.claude/CUSTOM.md` (under `## Deviations`
     if it overrides a harness rule, otherwise a section of its own). Shared
     with everyone who clones the repo only if `.claude/` is committed
     (`Harness: committed`). Never in `.claude/CLAUDE.md` — that file is
     synced and overwritten. If the user wants it personal-only, say so —
...
If unclear, default to a CLAUDE.md (`.claude/CUSTOM.md` in a container).
